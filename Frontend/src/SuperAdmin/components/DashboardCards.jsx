...
export default function DashboardCards({ stats }) {
  const totalUsersStr = String(stats?.totalUsers ?? 0);
  const activeUsersStr = String(stats?.activeUsers ?? 0);
  const platformsStr = String(stats?.supportedPlatformsCount ?? 0);
  ...
  const STAT_CONFIG = [
    ...
    { id: 'analyses',     title: 'Total Analyses',      value: String(stats?.totalAnalyses ?? 0), change: 'Live', color: '#7C3AED', icon: BarChart3 },
